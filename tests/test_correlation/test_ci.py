...
case_group_bias_adj = (
    [
        # correlation = 0.05 to 0.95 by 0.05, conf_level = 0.95, alternative = "two-sided", bias_adj = True
        TestCase(correlation=correlation, size=size, conf_level=0.95, interval_type="two-sided", bias_adj=True, distance=distance, actual_distance=actual_distance)
        for correlation, size, distance, actual_distance in [
            (0.05, 1530, 0.10, 0.099979332),
            (0.10, 1507, 0.10, 0.099984091),
            (0.15, 1469, 0.10, 0.099994088),
    ...
    + [
        # correlation = 0.05 to 0.95 by 0.05, conf_level = 0.95, alternative = "upper", bias_adj = True
        TestCase(correlation=correlation, size=size, conf_level=0.95, interval_type="upper", bias_adj=True, distance=distance, actual_distance=actual_distance)
        for correlation, size, distance, actual_distance in [
            (0.05, 268, 0.10, 0.099945111),
            (0.10, 262, 0.10, 0.099816657),
            (0.15, 252, 0.10, 0.099968113),
    ...
    + [
        # correlation = 0.05 to 0.95 by 0.05, conf_level = 0.95, alternative = "lower", bias_adj = True
        TestCase(correlation=correlation, size=size, conf_level=0.95, interval_type="lower", bias_adj=True, distance=distance, actual_distance=actual_distance)
        for correlation, size, distance, actual_distance in [
            (0.05, 274, 0.10, 0.099834696),
            (0.10, 272, 0.10, 0.099953148),
            (0.15, 268, 0.10, 0.099943989),
...
def test_solve_distance(case: TestCase) -> None:
    ...
    if case.bias_adj:
        pytest.skip("Currently no test cases available for comparison.")
